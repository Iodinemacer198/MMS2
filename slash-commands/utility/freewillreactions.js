...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const config = JSON.parse(fs.readFileSync('./freewillreact.json', 'utf8'));
        ...
        if (action === 'add') {
            if (!config.allowedChannels.includes(channelId)) {
                ...
                await interaction.reply(`Enabled free will reactions in this channel, with threshold ${minThreshold}-${maxThreshold}.`);
                fs.writeFileSync('./freewillreact.json', JSON.stringify(config, null, 4));
                return
            } else {
                return interaction.reply('This channel already has free will reactions enabled.');
            }
        }
        ...
        if (action === 'remove') {
            ...
            await interaction.reply('Disabled free will reactions in this channel.');
            fs.writeFileSync('./freewillreact.json', JSON.stringify(config, null, 4));
            return
        }
    }
